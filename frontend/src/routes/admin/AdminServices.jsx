...
import { ClipboardList, GripVertical, Search, X } from 'lucide-react'
...
function AdminServices() {
  ...
  const updateOrder = async (items) => {
    try {
      const updates = items.map((item, index) => 
        api.patch(`/admin/services/${item.id}/`, { order: index + 1 })
      )
  ...
  const handleEdit = (id, item) => {
    navigate(`/secret-admin-portal-2024/services/edit/${id}`)
  }
  ...
  const filteredServices = useMemo(() => {
    ...
    return filtered
  }, [services, searchValue, filterValue, categoryFilter, dateFilter, letterFilter])
  ...
  const confirmDelete = async () => {
    ...
    try {
      if (type === 'delete') {
        ...
        setServices(prev => prev.filter(item => item.id !== data.id))
        ...
        await Promise.all(itemsToDelete.map(id => api.delete(`/admin/services/${id}/`)))
  ...
  const categories = useMemo(() => {
    const cats = [...new Set(services.map(item => item.category).filter(Boolean))]
    return cats
  }, [services])

  const handleSelectAll = () => {
    selectAll(filteredServices)
  }
      ...
      <div className="container">
        <div className="admin-header">
          <div className="admin-header-buttons">
            <button
              onClick={() => navigate('/secret-admin-portal-2024/services/create')}
          ...
          <div className="admin-bulk-actions">
            ...
            <ActionsDropdown
              onEdit={() => {
                ...
                handleEdit(firstSelected, services.find(s => s.id === firstSelected))
              }}
              onDelete={handleBulkDelete}
              onView={() => {
                const firstSelected = Array.from(selectedItems)[0]
                handleView(firstSelected, services.find(s => s.id === firstSelected))
              }}
              itemId={Array.from(selectedItems)[0]}
              item={services.find(s => s.id === Array.from(selectedItems)[0])}
              showEdit={true}
              showDelete={true}
              showView={true}
            />
              ...
              {filteredServices.length === 0 ? (
                <tr>
                  <td colSpan="4" style={{ textAlign: 'center', padding: 'var(--spacing-xl)' }}>
                    {services.length === 0 ? 'No services found.' : 'No services match your search/filter criteria.'}
                  </td>
                </tr>
              ) : (
                filteredServices.map(service => (
                  <tr
                    key={service.id}
                    className={`admin-table-row ${selectedItems.has(service.id) ? 'row-selected' : ''} ${draggedItem?.id === service.id ? 'dragging' : ''} ${selectionMode ? 'selection-mode' : ''}`}
                    draggable
                    onDragStart={(e) => handleDragStart(e, service)}
                    onDragOver={handleDragOver}
                    onDrop={(e) => handleDrop(e, service)}
                    onDragEnd={handleDragEnd}
                    onMouseDown={() => handleLongPressStart(service.id)}
                    onMouseUp={handleLongPressEnd}
                    onMouseLeave={handleLongPressEnd}
                    onTouchStart={() => handleLongPressStart(service.id)}
                  ...
                  >
                    ...
                    <td>
                      {service.icon && typeof service.icon === 'string' && !/[\u{1F300}-\u{1F9FF}]/u.test(service.icon) ? (
                        <Icon name={service.icon} size={24} color="currentColor" />
                      ) : (
